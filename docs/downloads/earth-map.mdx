...
<button
    onClick={async () => {
        if (typeof umami !== 'undefined') {
            await umami.track('earth_map_1-326_download_torrent');
        }
        window.location.href = 'https://cdn.earthmc.org/downloads/earth-1-326.zip.torrent';
    }}
    style={{
        padding: '10px 20px',
        fontSize: '18px',
